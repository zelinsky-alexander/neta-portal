...
async function loadFindingSummary():Promise<FindingSummary>{
  ...
  for(let pageNo=0;pageNo<100;pageNo++){
    const page=await api<PageData<Finding>>(`/findings?limit=100&status=active${cursor?`&cursor=${encodeURIComponent(cursor)}`:''}`);
    for(const finding of page.items){
      if((finding.status??'').toUpperCase()!=='ACTIVE')continue;
      active++;
      const when=new Date(finding.lastSeen).getTime();
      if(Number.isFinite(when)&&(!oldestActive||when<new Date(oldestActive).getTime()))oldestActive=finding.lastSeen;
      const level=(finding.severity??'').toLowerCase() as keyof typeof severity;
      if(level in severity)severity[level]++;
    }
    if(!page.nextCursor||seenCursors.has(page.nextCursor))break;
    seenCursors.add(page.nextCursor);
    cursor=page.nextCursor;
  }
  return {active,oldestActive,...severity};
}
